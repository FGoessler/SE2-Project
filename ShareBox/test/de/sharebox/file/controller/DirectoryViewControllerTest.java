...
public class DirectoryViewControllerTest {
	...
	public void canCreateFilesBasedOnUsersSelection() {
		...
		when(controller.contextMenuController.getSelectedFEntry()).thenReturn(Optional.<FEntry>absent());
		...
		when(controller.contextMenuController.getSelectedFEntry()).thenReturn(Optional.<FEntry>of(subDir1));
		...
		when(controller.contextMenuController.getSelectedFEntry()).thenReturn(Optional.<FEntry>of(secondNewFile));
		when(controller.contextMenuController.getParentOfSelectedFEntry()).thenReturn(Optional.of(subDir1));
	...
	public void canCreateDirectoriesBasedOnUsersSelection() {
		...
		when(controller.contextMenuController.getSelectedFEntry()).thenReturn(Optional.<FEntry>of(subDir1.getFEntries().get(0)));
		when(controller.contextMenuController.getParentOfSelectedFEntry()).thenReturn(Optional.of(subDir1));
	...
	public void canReturnTheSelectedFEntriesAndTheirParents() {
		...
		assertThat(controller.getParentsOfSelectedFEntries()).contains(Optional.absent(), Optional.of(rootDirectory));
	}
